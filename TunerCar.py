...
class PurePursuitPlanner:
    ...
    def __init__(self, conf):
        self.conf = conf
        self.wheelbase = conf.l_r + conf.l_r
    ...
    def load_waypoints(self, conf):
        ...
        self.waypoints = np.loadtxt('example_waypoints.csv', delimiter=';', skiprows=3)
    ...
    def act(self, obs):
        ego_idx = obs['ego_idx']
        pose_theta = obs['poses_theta'][ego_idx] 
        pose_x = obs['poses_x'][ego_idx]
        pose_y = obs['poses_y'][ego_idx]
        # v_current = obs['linear_vels_x'][ego_idx]

        lookahead_distance = self.conf.lookahead
        vgain = self.conf.v_gain
